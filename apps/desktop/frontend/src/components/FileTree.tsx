 ...
 * - Teclado: ↑↓ navegar, Enter/→ abrir/expandir, ← colapsar/subir.
...
export function FileTree() {
  ...
  function onKeyDown(e: KeyboardEvent) {
    ...
    if (e.key === 'ArrowDown' || e.key === 'j') {
      ...
      if (item.isUp) { goUp(); return; }
      if (item.entry.is_dir && !expandedDirs().has(item.entry.name)) void toggleDir(item.entry.name);
    } else if (e.key === 'ArrowLeft' || e.key === 'h') {
      e.preventDefault();
      const item = items[idx];
      if (item.isUp) return;
      if (item.entry.is_dir && expandedDirs().has(item.entry.name)) {
        toggleDir(item.entry.name);
      } else if (!item.isUp) {
        goUp();
      }
