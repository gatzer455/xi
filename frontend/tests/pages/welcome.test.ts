...
const mock = vi.hoisted(() => {
  ...
  return {
    mockSignal,
    recentProjects: mockSignal([] as Array<{ name: string; path: string; lastOpened: number }>),
  };
});
...
describe('WelcomePage', () => {
  ...
  test('renders help link', () => {
    const page = WelcomePage();
    const help = page.root.querySelector<HTMLAnchorElement>('.welcome-help-link');
  ...
  test('shows recents when there are recent projects', () => {
    mock.recentProjects.value = [
      { name: 'proyecto-1', path: '/home/user/proyecto-1', lastOpened: Date.now() },
    ];
    const page = WelcomePage();
